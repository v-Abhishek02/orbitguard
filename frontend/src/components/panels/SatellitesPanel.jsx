...
export default function SatellitesPanel({ objects, onSelect }) {
  const [filter, setFilter] = useState('ALL');
  const sats  = objects.filter(o=>o.type==='PAYLOAD');
  const shown = sats.filter(o=>filter==='ALL'||o.risk===filter).slice(0,50);
  const S = {
  ...
  return (
    <div style={S.wrap}>
      <div style={S.title}>◉ ACTIVE SATELLITES</div>
      <div style={S.sub}>{sats.length.toLocaleString()} PAYLOAD objects</div>
      <div style={S.filter}>
